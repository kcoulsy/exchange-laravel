{{-- @if (!Cookie::get('cookie_consent'))
    <div x-data="{ open: true }" class="min-h-screen bg-gray-100 py-6 flex flex-col justify-center sm:py-12">
        <div x-show="open" x-transition:enter-start="opacity-0 scale-90"
            ...
            class="max-w-screen-lg mx-auto fixed bg-white inset-x-5 p-5 bottom-4 lg:bottom-10 rounded-lg drop-shadow-2xl flex gap-4 flex-wrap md:flex-nowrap text-center md:text-left items-center justify-center md:justify-between">
            ...
            <div class="flex gap-4 items-center flex-shrink-0">
               ...
               so that the popup won't appear next time they load. -->
                <button @click="open = false, denyCookies()"
                    class="text-indigo-600 focus:outline-none hover:underline">Decline</button>
                <button @click="open = false, allowCookies()"
